...
CString strMail;
CStringArray black_list;
CStringArray white_list;
CString USER;
...
BOOL CSMTPDlg::OnInitDialog()
{
	...
	strText = _T("邮件主题");
	m_listmail.InsertColumn(1,strText,LVCFMT_CENTER,200);
	strText = _T("发送方");
	m_listmail.InsertColumn(2,strText,LVCFMT_CENTER,200);
	strText = _T("大小");
	m_listmail.InsertColumn(3,strText,LVCFMT_CENTER,50);
	USER="";
...
void CSMTPDlg::OnGetemail() 
{
	...
	for(UINT i=0;i<n;++i)
	{
		...
		bool flag=1;
		for(int j=0;j<black_list.GetSize();++j)
			if(tmp==black_list.GetAt(j)) {flag=0;break;}
		if(!flag) continue;
